...
(function () {
  ...
  /* ---------------- Featured: fullscreen machine posters ----------------
     ...
     arrows, the keyboard) does the same thing to it: step it by
  ...
  function initMachinePosters() {
    ...
    /* --- Giant-title fitting -------------------------------------------
       Names differ wildly in length ("ECO 500" against "Sugarcane Juicer"),
       so NOTHING here may be tuned for one of them: every name is measured
       on its own and solved for the size that makes it span the same span
       of the section as every other name.

       Measurement notes, both of which matter for getting all six to land
       identically rather than just one:
        - getBoundingClientRect().width, not scrollWidth. scrollWidth is a
          rounded integer and drops the trailing negative letter-spacing,
          which is worth several pixels at these sizes and pushed short
          names visibly off the target the long ones hit.
        - the span is measured against the STAGE, not the window, so the
          name aligns with the section it lives in at any width.

       The old height cap (46vh) was what actually broke consistency: a
       short name needs a very large size to reach edge to edge, so it hit
    ...
    function fitTitles() {
      ...
      parts.forEach(function (p) {
        if (!p.word) return;
        var want = box * target;
        var cap  = window.innerHeight * .92;

        /* Solve by measuring rather than by formula. A single probe-and-
           scale assumes width is exactly linear in font-size, and it is
           not: hinting, and the letter-spacing applied after the final
           glyph, both shift the box by an amount that does not scale
           cleanly. That left the names landing on widths ~25px apart —
           small, but enough to read as "one product is aligned and the
           rest are not".

           So the first pass estimates, and two correction passes re-measure
           at the estimate and scale by the error. This converges to within
           a fraction of a pixel for ANY name, which is what keeps all six
           on the same span. */
        var prev = p.word.style.fontSize;
        p.word.style.fontSize = FIT_PROBE + "px";
        var size = FIT_PROBE * want / (inkWidth(p) || 1);

        /* Converge on the WIDTH first, uncapped. Clamping inside the loop
           let the height cap stop the correction early: the pass measured
           at the capped size and then scaled from there, so a name that hit
           the cap never reached the target span and ran out to the edges
           (94-100.3% of the viewport at 1920, against the 88% intended —
           one name was 2.5px off-screen on each side). The cap is a
           separate limit and is applied once, after the fit has settled. */
    ...
    function measure() {
      ...
      /* A BRIEF hold, not a long one. 2.4 gave ~1.4 screens of sticky
         travel: the poster locked the viewport for several holds and the
         page felt stuck — the visitor kept scrolling and nothing moved.

         1.35 gives ~.35 of a screen of pin. The section still catches and
         settles for a moment as it arrives (so a machine owns the screen
         rather than sliding past mid-transition), and the very next scroll
         gesture releases it and the page carries on normally. Autoplay is
         on a timer, so a machine still changes during that hold; it just
         no longer costs the visitor a screen and a half of scrolling. */
      track.style.height = Math.round(VH * 1.35) + "px";
      fitTitles();
    }
    ...
    function drawEnter(p, t) {
      ...
      if (p.mach) {
        ...
        /* The X term keeps --ink-off (set per scene in the CSS) so a
           cut-out with uneven transparent padding stays centred on its
           visible machine while it animates. Without it this inline
           transform would overwrite the CSS correction on the first
           frame and the machine would jump off centre. */
        p.mach.style.transform =
          "translate3d(calc(-50% - var(--ink-off, 0%)), calc(-50% + " + y.toFixed(1) + "px), 0) scale(" + scale.toFixed(3) + ")";
    ...
    function drawExit(p, t) {
      ...
      if (p.mach) {
        p.mach.style.transform =
          "translate3d(calc(-50% - var(--ink-off, 0%)), calc(-50% + " + (-e * 30).toFixed(1) + "px), 0) scale(" + (1 - e * .07).toFixed(3) + ")";
      }
      if (p.word) {
        p.word.style.transform =
    ...
    /* Re-fit once the real webfont is in. The names are fitted by MEASURING
       their ink, so the measurement is only as good as the font that was
       active when it ran: at init 'General Sans' is usually still loading
       and the probe lands on fallback metrics, whose glyph widths differ
       per name. The fit then holds those wrong sizes until something else
       calls measure() — which is why the same build showed a clean, uniform
       span at some widths and a per-product spread (85-98% of the section
       instead of the intended share) at others, and why a resize "fixed"
       it. Refitting on fonts.ready makes the first paint match every later
       one. Guarded: document.fonts is absent on older browsers, where the
       init fit stays as the only one. */
    if (document.fonts && document.fonts.ready && document.fonts.ready.then) {
      document.fonts.ready.then(function () { fitTitles(); });
    }
  ...
  function initWhyReveal() {
    ...
    /* ---- Parallax: removed ----
       The two photographic layers used to be re-transformed on every scroll
       frame — the wall drifting at one rate, the machine at another — which
       is what made the images travel as the section moved. Both layers are
       now left exactly where the stylesheet puts them, so the composition is
       a still photograph that scrolls with the page.

       onScroll survives as a no-op because the resize handler above and the
       observer both call it; keeping it means neither has to know the
       parallax is gone. */
    function onScroll() {}
  }
  ...
  /* Drives ScrollTrigger off Lenis' own rAF loop and reports Lenis' smoothed
     position as the scroll position. Both initParallax and initTwoRanges may
     need it, and registering the ticker callback twice would advance Lenis
     twice per frame, so this only ever wires up once. */
  var lenisBridged = false;
  function bridgeLenisToGsap() {
    if (lenisBridged || !lenis) return;
    lenisBridged = true;
  ...
  /* Image parallax — runs on any page, independent of the homepage's
     Two Ranges panels. Needs GSAP + ScrollTrigger; without them the images
     simply sit still, which is the correct fallback. */
  function initParallax() {
    ...
    bridgeLenisToGsap();
  /* ---------------- Image parallax ----------------
     Any element carrying data-parallax drifts against the scroll while its
     container crosses the viewport. The container is the nearest
     [data-parallax-container] (or the parent), and travel is a FRACTION OF
     THAT CONTAINER'S HEIGHT: the element runs from -speed*h to +speed*h,
     linear, scrubbed to scroll position.

     The element must be taller than its container by 2*speed, or the drift
     exposes an edge — .range-feature img is inset:0 with height:100%, so
     the CSS gives those the extra height (see --px-travel).

     Both functions resolve their values lazily (as callbacks) so a resize
     or a late-decoding image re-measures instead of keeping stale pixels. */
  gsap.utils.toArray("[data-parallax]").forEach(function (el) {
    var speed = parseFloat(el.dataset.parallax);
    if (!isFinite(speed) || speed === 0) return;
    var box = el.closest("[data-parallax-container]") || el.parentElement || el;
    gsap.fromTo(el,
      { y: function () { return -speed * box.offsetHeight; } },
      { y: function () { return  speed * box.offsetHeight; },
        ease: "none",
        scrollTrigger: {
          trigger: box,
  ...
  function initTwoRanges() {
    ...
    function playWhenVisible(video) {
      function warm() {                          // start fetching before it is needed
      ...
      /* Two observers. The outer one (a viewport-height margin, any sliver)
         only warms the buffer so the clip has data by the time the panel
         pins; the inner one plays and pauses. Without the warm-up pass the
         panel arrives pinned and full-bleed while the file is still being
         fetched, which reads as a video stuck on its poster. */
      new IntersectionObserver(function (entries) {
        entries.forEach(function (e) { if (e.isIntersecting) warm(); });
      ...
      /* A play() issued with no buffered data can reject or simply not
         advance; once the browser has frames, try again if it is on screen
         and still stalled. */
      video.addEventListener("loadeddata", function () {
        if (video.paused && isOnScreen(video)) start();
      });
    ...
    // Drive ScrollTrigger off Lenis' own frame (see bridgeLenisToGsap).
    bridgeLenisToGsap();
